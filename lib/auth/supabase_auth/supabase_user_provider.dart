...
class DigitalClinicSupabaseUser extends BaseAuthUser {
  DigitalClinicSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> digitalClinicSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DigitalClinicSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
