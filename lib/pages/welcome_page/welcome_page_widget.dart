  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      if (valueOrDefault(currentUserDocument?.role, '') == 'patient') {
        ...
        if (valueOrDefault(currentUserDocument?.role, '') == 'relative') {
          context.pushNamed('HomePageRelative');
        } else {
                ...
                if (valueOrDefault(currentUserDocument?.role, '') == 'ceo') {
                  ...
                  if (valueOrDefault(currentUserDocument?.role, '') ==
                    ...
                    if (valueOrDefault(currentUserDocument?.role, '') ==
                      ...
                      if (valueOrDefault(currentUserDocument?.role, '') ==
                          'registered') {
                        context.pushNamed('HomePageRegistered');
                      } else {
                        if (valueOrDefault(currentUserDocument?.role, '') ==
                            'receptionist') {
                          context.pushNamed('HomePageReceptionist');
                        } else {
                          if (currentUserEmail != null &&
                              currentUserEmail != '') {
                            context.pushNamed('HomePageLoggedNoRole');
                          } else {
                            context.pushNamed('HomePageLoggedPhoneNoRole');
                          }
                        }
                      }
                    }
