...
import 'nur_patients_model.dart';
export 'nur_patients_model.dart';

class NurPatientsWidget extends StatefulWidget {
  const NurPatientsWidget({super.key});

  @override
  State<NurPatientsWidget> createState() => _NurPatientsWidgetState();
}

class _NurPatientsWidgetState extends State<NurPatientsWidget>
    with TickerProviderStateMixin {
  late NurPatientsModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => NurPatientsModel());
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      if (FFAppState().idsList.contains(currentUserUid)) {
        ...
        await actions.subscribeRealtimeFilter(
          ...
          () async {
            _model.calls11Copy = await PatientsCallsViewTable().queryRows(
              queryFn: (q) => q
                  .eq(
                    'is_completed',
                    false,
            ...
            _model.numOfCalls = _model.calls11Copy?.length;
            setState(() {});
          },
        );
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<PNursesRow>>(
      ...
      builder: (context, snapshot) {
        ...
        List<PNursesRow> nurPatientsPNursesRowList = snapshot.data!;

        // Return an empty Container when the item does not exist.
        if (snapshot.data!.isEmpty) {
          return Container();
        }
        final nurPatientsPNursesRow = nurPatientsPNursesRowList.isNotEmpty
            ? nurPatientsPNursesRowList.first
            : null;
              ...
              child: Drawer(
                ...
                child: wrapWithModel(
                  ...
                  child: NurDrawerWidget(
                    selectedPage: 2,
                    userData: nurPatientsPNursesRow!,
                  ),
                ),
              ),
            ),
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Flexible(
                                          child: wrapWithModel(
                                            model: _model.patientListNurModel,
                                            updateCallback: () =>
                                                setState(() {}),
                                            child: PatientListNurWidget(
                                              department: nurPatientsPNursesRow!
                                                  .departmentId!,
                                            ),
                                          ),
                                        ),
