...
import 'nur_surgeries_schedule_model.dart';
export 'nur_surgeries_schedule_model.dart';

class NurSurgeriesScheduleWidget extends StatefulWidget {
  const NurSurgeriesScheduleWidget({super.key});

  @override
  State<NurSurgeriesScheduleWidget> createState() =>
      _NurSurgeriesScheduleWidgetState();
}

class _NurSurgeriesScheduleWidgetState extends State<NurSurgeriesScheduleWidget>
    with TickerProviderStateMixin {
  late NurSurgeriesScheduleModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => NurSurgeriesScheduleModel());
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      if (FFAppState().idsList.contains(currentUserUid)) {
        _model.calls = await LogPatientsCallsTable().queryRows(
          queryFn: (q) => q
              .eq(
                'is_completed',
                false,
        ...
        await actions.subscribeRealtimeFilter(
          ...
          () async {
            _model.calls11 = await LogPatientsCallsTable().queryRows(
              queryFn: (q) => q
                  .eq(
                    'is_completed',
                    false,
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<PNursesRow>>(
      ...
      builder: (context, snapshot) {
        ...
        List<PNursesRow> nurSurgeriesSchedulePNursesRowList = snapshot.data!;

        // Return an empty Container when the item does not exist.
        if (snapshot.data!.isEmpty) {
          return Container();
        }
        final nurSurgeriesSchedulePNursesRow =
            nurSurgeriesSchedulePNursesRowList.isNotEmpty
                ? nurSurgeriesSchedulePNursesRowList.first
                : null;
              ...
              child: Drawer(
                ...
                child: wrapWithModel(
                  ...
                  child: NurDrawerWidget(
                    selectedPage: 6,
                    userData: nurSurgeriesSchedulePNursesRow!,
                  ),
                ),
              ),
            ),
