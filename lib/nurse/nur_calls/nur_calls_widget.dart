...
import 'nur_calls_model.dart';
export 'nur_calls_model.dart';

class NurCallsWidget extends StatefulWidget {
  const NurCallsWidget({super.key});

  @override
  State<NurCallsWidget> createState() => _NurCallsWidgetState();
}

class _NurCallsWidgetState extends State<NurCallsWidget>
    with TickerProviderStateMixin {
  late NurCallsModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => NurCallsModel());
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<PNursesRow>>(
      ...
      builder: (context, snapshot) {
        ...
        List<PNursesRow> nurCallsPNursesRowList = snapshot.data!;

        // Return an empty Container when the item does not exist.
        if (snapshot.data!.isEmpty) {
          return Container();
        }
        final nurCallsPNursesRow = nurCallsPNursesRowList.isNotEmpty
            ? nurCallsPNursesRowList.first
            : null;
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Expanded(
                                          child: wrapWithModel(
                                            model:
                                                _model.patientListCallsNurModel,
                                            updateCallback: () =>
                                                setState(() {}),
                                            child: PatientListCallsNurWidget(
                                              nurDepartment:
                                                  valueOrDefault<int>(
                                                _model.userDataRows?.first
                                                    ?.departmentId,
                                                0,
                                              ),
                                              numOfCallsCallback:
                                                  (numCalls) async {
                                                _model.numOfCalls = numCalls;
                                                setState(() {});
                                              },
                                            ),
                                          ),
